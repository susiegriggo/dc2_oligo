...
def train_dimer(
    ...
    """
    Train an ensemble of logistic regression models to predict if a protein is a dimer or some other homomer 
    ...
    # Correct the check for monomers
    if (df['chains'] == 1).any():
        raise ValueError("Training data contains monomers (chains == 1). Please provide data with only homomers.")

    # Create a binary state column based on the number of chains
    df['state'] = [1 if i != 2 else 0 for i in df['chains']]
    # Encode the state labels
    le = LabelEncoder()
    df['y'] = le.fit_transform(df['state'])
    print(df)
    ...
    logger.info("Starting dimer training")
    ...
    df["prob_other"] = results.mean(axis=0).mean(axis=0)[:, 1]
    ...
    logger.info(f"Dimer training completed with results: {results_}")
...
def train_trimer(
    ...
    """
    Train an ensemble of logistic regression models to predict if a protein is a trimer or some other homomer 
    ...
    # Create a binary state column based on the number of chains
    df['state'] = [1 if i != 3 else 0 for i in df['chains']]
    # Encode the state labels
    le = LabelEncoder()
    df['y'] = le.fit_transform(df['state'])
    print(df)
    ...
    logger.info("Starting trimer training")
    ...
    results_["f1"] = f1_score(y, y_pred_bin, average='macro')
    df["y_pred"] = y_pred_bin
    df["prob_trimer"] = results.mean(axis=0).mean(axis=0)[:, 0]
    df["prob_other"] = results.mean(axis=0).mean(axis=0)[:, 1]
    ...
    logger.info(f"Trimer training completed with results: {results_}")
...
@click.option('--kind', type=click.Choice(['monomer', 'dimer', 'trimer', 'homomer', 'ordinal_homomer']), default='monomer', help='Type of training (monomer, dimer, trimer, homomer, or ordinal_homomer)')
...
def main(af2_embeddings_dir, kind, regularization, dual, balanced, ensemble_size, use_scaler, use_pairwise, output_dir, training_data):
    try:
        ...
        if kind == 'monomer':
            ...
            results, model, df = train_dimer(training_data, af2_embeddings_dir, regularization, balanced, dual,
                                             ensemble_size, use_pairwise, use_scaler, output_dir)
        elif kind == 'trimer':
            results, model, df = train_trimer(training_data, af2_embeddings_dir, regularization, balanced, dual,
                                              ensemble_size, use_pairwise, use_scaler, output_dir)
        elif kind == 'homomer':
